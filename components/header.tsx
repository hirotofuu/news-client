...
const Header=()=>{
  ...
  const [lognin, setLogin]=useState<boolean>(false)
  ...
  return(
    <>
      <div className="flex justify-between bg-white h-12 items-center border-b-2">
        ...
        <Link href="/"><a className="xl:text-4xl lg:text-4xl md:text-4xl sm:text-4xl text-3xl lg:ml-6 xl:ml-6 md:ml-6 sm:ml-6 ml-1">Nite</a></Link>
          ...
          <div className="gap-5 hidden xl:flex lg:flex md:flex sm:flex items-center">
            ...
            <Link href="/login"><a className="flex items-center mr-10">
              <FontAwesomeIcon icon={faCircleUser} className="text-2xl"/>
              <h1 className="text-2xl mb-2 ml-1">{lognin ? user.name : ''}</h1>
              </a></Link>
          </div>
